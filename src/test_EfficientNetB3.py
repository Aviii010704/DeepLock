...
class TestConfig:
    ...
    MODEL_PATH = os.path.join(MODEL_DIR, "deeplock_EfficientNet.pth")
    
    # Output directories
    RESULTS_DIR = "test_results/efficientnet"
...
def get_efficientnet(pretrained=False):
    """Load EfficientNet-B3 model with custom classifier"""
    model = create_model(
        "efficientnet_b3",
...
def load_model(device):
    ...
    print(f"{Fore.CYAN}📥 Loading EfficientNet model from {TestConfig.MODEL_PATH}...{Style.RESET_ALL}")
    ...
    try:
        ...
        print(f"{Fore.GREEN}✅ EfficientNet-B3 model loaded successfully!{Style.RESET_ALL}")
        return model
    
    except Exception as e:
        print(f"{Fore.RED}❌ Error loading EfficientNet model: {str(e)}{Style.RESET_ALL}")
        raise

def test_model(model, test_loader, device):
    """Test the model and calculate metrics"""
    print(f"\n{Fore.MAGENTA}🔍 Starting EfficientNet model evaluation...{Style.RESET_ALL}")
    ...
    # Print metrics
    print(f"\n{Fore.GREEN}✨ EfficientNet Test Results:{Style.RESET_ALL}")
...
def plot_metrics_bar(metrics):
    ...
    plt.xlabel('Metrics')
    plt.ylabel('Score')
    plt.title('EfficientNet Performance Metrics')
...
def main():
    ...
    print(f"\n{Fore.GREEN}✅ EfficientNet testing completed!{Style.RESET_ALL}")
